    ...
    if (isAdministrator) {
        ...
        if(pass.length != 0)
        {
            ...
            if(userName.length != 0)
                userInfo = [userInfo stringByAppendingFormat:@"user name \"%@\" ", userName];
            
            userInfo = [userInfo stringByAppendingFormat:@"password \"%@\"", pass];
            script = [NSString stringWithFormat:@"do shell script \"%@\" %@ with administrator privileges", fullScript, userInfo];
        }
        else
...
+ (BOOL) isModuleLoaded  {
    ...
    [self runProcess:@"kextstat | grep com.nanoant.DisableTurboBoost"
       withArguments:[NSArray arrayWithObjects:@"", nil]
              output:&output
    errorDescription:&processErrorDescription asAdministrator:NO];
...
//    }
    // 64 bits only
    modulePath = [[NSBundle mainBundle] pathForResource:@"DisableTurboBoost" ofType:@"kext"];
